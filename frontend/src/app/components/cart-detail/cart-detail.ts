...
export class SalesHistoryComponent implements OnInit {
  ...
  // =========================================================
  // MÉTODO CORREGIDO: Recibe Blob directamente
  // =========================================================
  descargarRecibo(saleId: string) {
    this.downloadingSaleId.set(saleId);
    
    // Ahora esperamos un Blob (archivo), no un JSON con base64
    this.salesService.getReceipt(saleId).subscribe({
      next: (blob: Blob) => {
        // Creamos la URL temporal para el archivo recibido
        const url = window.URL.createObjectURL(blob);
        
        // Creamos un link invisible y lo clicamos
        const a = document.createElement('a');
        a.href = url;
        a.download = `recibo-${saleId}.pdf`;
        a.target = '_blank'; // Opcional: abrir en nueva pestaña
      ...
      error: (err) => {
        ...
        // alert('No se encontró el recibo para esta venta.');
        this.downloadingSaleId.set(null);
      }
    });
  }
  ...
  // La función 'base64ToBlob' ha sido eliminada porque ya no se necesita.
}
